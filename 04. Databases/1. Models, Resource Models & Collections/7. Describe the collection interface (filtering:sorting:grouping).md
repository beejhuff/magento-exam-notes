...
Both methods accept 2 parameters - field and the condition which could be a string or an array. These two methods effectively add a WHERE clause to the SQL query being used behind the scenes.
...
```
    ...
    $collection = $this->getCollection()->setOrder('fieldname', 'asc');
```
